...
module.exports = function (ast) {
  ...
  ast.QualifiedNameNode.prototype.build = function (args) {
    ...
    return new Promise((resolve, reject) => {
      ...
      first.build(args).then((firstResult) => {
        ...
        .catch((err) => {
          // //rlog.error(options, `QualifiedNameNode build failed with error - ${err},text: ${this.text}`);
          // r//rlog.error(options, 'QualifiedNameNode build failed', this.rule, this.text, err);
          reject(err);
        });
    });
  };
    ...
    return new Promise((resolve, reject) => {
      Promise.all([this.operand_1, this.operand_2].map((d) => {
        ...
        .then(([first, second]) => {
          ...
          // //rlog.debug(options, `ArithmeticExpressionNode build success with result - ${stringify(result)}, text: ${this.text}`);
          // r//rlog.debug(options, 'ArithmeticExpressionNode build success', this.rule, this.text, stringify(result));
          resolve(result);
        })
        .catch((err) => {
  ...
  ast.LiteralNode.prototype.build = function (args) { // eslint-disable-line no-unused-vars
    // const options = (args && args.context && args.context.options) || {};
